...
#                                                                                                  #
#  Modification date: 10/5/20 22:14                                                                #
#                                                                                                  #
# ##################################################################################################
...
def bond_angle(a, b, c):
    ...
    if dist == 0:  # SD 2010 - http://mgl.scripps.edu/forum/viewtopic.php?f=11&t=245&p=1882
        # FIXED: Return default angle instead of crashing (terminal atoms issue)
        return 120.0  # tetrahedral-ish default angle
    ...
    if cos_theta + 1.0 < 0.0001:
        ...
        angle = (math.acos(cos_theta)) * RAD_TO_DEG
    return angle
...
def vec3(a, b, norm=1.0):
    ...
    # FIXED: Handle zero-length vectors (identical points)
    if length == 0:
        return [0.0, 0.0, norm]  # Return default unit vector in z direction
    l = norm / length
